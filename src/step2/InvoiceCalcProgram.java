...
public class InvoiceCalcProgram {
	...
	public static void main(String[] args) {
		...
		try {
			...
			while ((line = bufferedReader.readLine()) != null) {
				...
				case '1':
					// src_tel_num = list[1];
					src_tel_num = getOwnerTelNumber(line);
					break;
				// 1桁目が2の場合は加入サービス情報を取得
				case '2':
					base_charge = calcBaseCharge(line, base_charge);
					family_tel_num_array.add(getServiceOption(line));
					break;
				case '5':
					// call_date = list[1];
					call_charge_total += calcCallCharge(line, base_charge, family_tel_num_array);
					break;
	...
	private static int calcCallCharge(String line, int base_charge, ArrayList<String> family_tel_num_array) {
		...
		try {
			String call_time_str = getStartHour(line);
			int call_duration = getCallMinutes(line);
			String dst_tel_num = getCallNumber(line);
			int call_charge = 20;
			SimpleDateFormat sdFormat = new SimpleDateFormat("hh:mm");
			Date call_time;
			call_time = sdFormat.parse(call_time_str);
			...
			if (base_charge == 1300) {
				System.out.println(start_time.compareTo(call_time) < 0 && 0 < end_time.compareTo(call_time));
				System.out.println("start:" + start_time);
				System.out.println("end:" + end_time);
				System.out.println("call:" + call_time);
				...
				System.out.println(family_tel_num_array.contains(dst_tel_num));
	...
	private static String getCallNumber(String line) {
		String[] list = line.split(" ");
		String dst_tel_num = list[4];
		return dst_tel_num;
	}

	private static int getCallMinutes(String line) {
		String[] list = line.split(" ");
		int call_duration = Integer.parseInt(list[3]);
		// TODO 自動生成されたメソッド・スタブ
		return call_duration;
	}

	private static String getStartHour(String line) {
		String[] list = line.split(" ");
		String call_time_str = list[2];
		return call_time_str;
	}

	private static String getServiceOption(String line) {
		String[] list = line.split(" ");
		String family_tel_num = null;
		if (list[1].charAt(0) == 'C') {
			family_tel_num = list[2];
		}
		return family_tel_num;
	}
